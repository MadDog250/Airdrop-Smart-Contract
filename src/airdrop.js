  ...
  csv.parseStream(stream)
    ...
    .on('data', row => {
      ...
      if (isAddress && row[0] != null && row[0] != '') {
       ...
       // allocAmountData.push(web3.utils.toWei( row[1]));
        ...
        index++;
        if (index >= BATCH_SIZE) {
          console.log("alloc length: ", allocAddressData.length);
          distribAddressData.push(allocAddressData);
         // distribAmountData.push(allocAmountData);
          //allocAmountData = [];
          console.log("length: ", distribAddressData.length);
          allocAddressData = [];
          index = 0;
        }
      }  
    ...
    .on('end', rowCount => {
     ...
     // allocAmountData = [];
      ...
      //withdrawTokens();
      console.log("pre airdrop length: ", distribAddressData.length);
      airDrop();
    });
...
const airDrop = async () => {
  ...
  for (var i = 0; i < distribAddressData.length; i++) {
    ...
    try {
      ...
      let r = await myContract.methods.dropTokens(distribAddressData[i], distribAmountData).send({ from: accounts[0], gas: gas, gasPrice: gPrice });
      console.log('------------------------')
